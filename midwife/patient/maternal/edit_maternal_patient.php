...
<div class="container-fluid py-4">
    <div class="row g-0 border border-secondary-subtle rounded bg-white">
            ...
            <div class="d-flex flex-column h-100">
                <div class="mb-4 ps-2">
                    ...
                    <h5 class="text-dark fw-bold mb-0">
                        <?=htmlspecialchars(($patient['first_name'] ?? '') . ' ' . ($patient['last_name'] ?? '')) ?: 'Maternal Patient'?>
                    </h5>
                    <small class="text-secondary font-monospace">ID: <?=htmlspecialchars($patient['patient_id'] ?? 'N/A')?></small>
                </div>
                
                <!-- Tab Triggers -->
                <div class="nav flex-column nav-pills gap-1" id="maternalFormTabs" role="tablist" aria-orientation="vertical">
        ...
        <div class="col-md-9 d-flex flex-column">
            <form id="editMaternalForm" method="POST" class="d-flex flex-column justify-content-between h-100 mb-0">
                <input type="hidden" name="patient_id" value="<?=htmlspecialchars($patient['patient_id'] ?? '')?>">
                
                <div class="card-body p-4 tab-content" id="maternalFormContent">
                    
                    <!-- PANEL 1: Identity -->
                    <div class="tab-pane fade show active" id="panel-identity" role="tabpanel" aria-labelledby="tab-identity">
                        <h6 class="fw-bold text-dark border-bottom pb-2 mb-4">Patient Identity</h6>
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label small fw-medium text-secondary">First Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="first_name" value="<?=htmlspecialchars($patient['first_name'] ?? '')?>" required>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-medium text-secondary">Middle Name</label>
                                <input type="text" class="form-control" name="middle_name" value="<?=htmlspecialchars($patient['middle_name'] ?? '')?>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-medium text-secondary">Last Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="last_name" value="<?=htmlspecialchars($patient['last_name'] ?? '')?>" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="panel-demographics" role="tabpanel" aria-labelledby="tab-demographics">
                        <h6 class="fw-bold text-dark border-bottom pb-2 mb-4">Clinical Demographics</h6>
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                <label class="form-label small fw-medium text-secondary">Date of Birth</label>
                                <input type="date" class="form-control" name="birth_date" max="<?=date('Y-m-d')?>" value="<?=htmlspecialchars($patient['birth_date'] ?? '')?>" id="birth_date">
                                <span id="error_birth_date" class="text-danger small"></span>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-medium text-secondary">Age <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" min="0" max="120" name="age" value="<?=htmlspecialchars($patient['age'] ?? '')?>" id="age" required>
                            </div>
                        </div>
                        <div class="mb-4">
                            <label class="form-label small fw-medium text-secondary d-block mb-2">Age Bracket</label>
                            <div class="d-flex gap-3">
                                <div class="form-check border rounded px-3 py-2 flex-fill custom-radio-card">
                                    <input class="form-check-input ms-0 me-2" type="radio" name="age_bracket" value="10-14" id="age_bracket_1" <?=($patient['age_bracket'] ?? '') == '10-14' ? 'checked' : ''?>>
                                    <label class="form-check-label text-dark small" for="age_bracket_1">10–14 y/o</label>
                                </div>
                                <div class="form-check border rounded px-3 py-2 flex-fill custom-radio-card">
                                    <input class="form-check-input ms-0 me-2" type="radio" name="age_bracket" value="15-19" id="age_bracket_2" <?=($patient['age_bracket'] ?? '') == '15-19' ? 'checked' : ''?>>
                                    <label class="form-check-label text-dark small" for="age_bracket_2">15–19 y/o</label>
                                </div>
                                <div class="form-check border rounded px-3 py-2 flex-fill custom-radio-card">
                                    <input class="form-check-input ms-0 me-2" type="radio" name="age_bracket" value="20-49" id="age_bracket_3" <?=($patient['age_bracket'] ?? '') == '20-49' ? 'checked' : ''?>>
                                    <label class="form-check-label text-dark small" for="age_bracket_3">20–49 y/o</label>
                                </div>
                            </div>
                        </div>
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="date" class="form-control" name="date_of_registration" max="<?=date('Y-m-d')?>" value="<?=htmlspecialchars($patient['date_of_registration'] ?? '')?>" id="date_of_registration">
                                <span id="error_date_of_registration" class="text-danger small"></span>
                            </div>
                            <div class="col-md-4">
                ...
                <div class="d-flex justify-content-end gap-2 p-3 bg-light border-top mt-auto">
                    <button type="button" class="btn btn-sm btn-outline-secondary px-4 fw-medium" onclick="loadPage('patient/maternal/view_maternal_patient.php');">Back</button>
...
<style>
/* Clean override styles to remove generic framework bloat */
#maternalFormTabs .nav-link {
    color: var(--bs-secondary);
    border: 1px solid transparent;
}
#maternalFormTabs .nav-link.active {
