...
// check kung pinindot yung submit button
if (isset($_POST['submit_btn'])) {
    // check kung via POST method yung pagsubmit
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        //before makapag insert, check muna kung may existing na record
        ...
        if ($result_check->num_rows > 0) {
            $_SESSION['statusMessage'] = "Maternal Record already exists!";
        ...
        try {
          ...
          // === AUTO-CREATE Patient USER  ==
            ...
            $sql_patient = file_get_contents('../../../queries/maternal_insert/insert_maternal_patient.sql'); //separate sql file to
            $stmt_patient = $conn->prepare($sql_patient);

            $registered_by_midwife_id = (int)$_SESSION['user_id'];
            $health_center_id = (int)$_SESSION['health_center_id'];

            $patient_type = 'mother'; //para maindicate na for mother itong record haha
            ...
            "iisssssssssssisssi",
            $patient_user_id,              // <-- bagong Patient user_id galing sa insert ng USER
            $registered_by_midwife_id,
            $patient_type,
            $mother_id,
            ...
            $email_for_user,               // <-- pareho sa login username/email na ginawa mo
            $health_center_id
            );
            ...
            $patient_id = $conn->insert_id; //since auto increment yung primary key, purpose nito is makuha yung id number
            $stmt_patient->close();
            ...
            $_SESSION['statusMessageCode'] = "error";
             header("Location: add_maternal.php");  // same folder as maternal_process.php
             exit();
        }
    }
} else {
