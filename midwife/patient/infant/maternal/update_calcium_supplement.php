...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_query = "UPDATE maternal_supplements SET supp_trimester = ?, supp_tablets_given = ?, date_supp = ? WHERE maternal_supplement_id = ? AND pregnancy_id = ? AND supplement_type = 'Calcium Carbonate'";
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            echo json_encode(["success" => true, "message" => "Calcium supplement updated successfully"]);
        } else {
